...
void generujMenu(){
	string elementy[] = { "Wypisz arkusz", "Modyfikuj element", "Zmień rozmiar", "Utwórz nowy arkusz", "Zapisz arkusz", "Wczytaj arkusz", "Wyświetl parametry", "Zmień typ kolumny", "Wyjdź z programu"};
	int i{};
	for(auto x: elementy){
		cout << ++i << ". " << x << endl;
	}
...
void parametry(Arkusz arkusz){
	if(!arkusz.hasCalculateableColumn()){
...
string parametryWiersza(Arkusz arkusz, int wiersz){
	...
	ss << "Średnia elementów wiersza: " << (static_cast<double>(suma)/arkusz.calculateableColumnsCount()) << endl;

	return ss.str();
}

string parametryKolumny(Arkusz arkusz, int kolumna){
	if(arkusz.calculateableColumn(kolumna)){
	...
	return "Wybrana kolumna jest typu tekstowego";
}
...
void zmienKolumne(Arkusz *arkusz){
	cout << "Podaj kolumnę którą chcesz zmodyfikować: ";
	size_t wspY;
	wspY = wprowadzZakres(0, arkusz->rozmiarX()-1);
	
	cout << "Podaj typ na który chcesz zmienić kolumnę (0. int) (1. string): ";
	int typWpr;
	typWpr = wprowadzZakres(0, 1);

	cellType typ = cellType::typeInt;
	if(typWpr == 1){
		typ = cellType::typeString;
	} 

	arkusz->convertColumn(typ, wspY);
}